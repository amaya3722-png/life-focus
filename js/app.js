(function(){
  ...
  function addQuick(){var input=document.getElementById('quickTask'),name=input.value.trim();if(!name)return;var id=Date.now();tasks.push({id:id,sourceKey:'local:'+id,name:name,subject:'其他',plannedDuration:45,actualDuration:0,status:'pending',date:selectedDate});input.value='';saveData();renderTaskList();if(window.TimeBlock)TimeBlock.refresh()}
